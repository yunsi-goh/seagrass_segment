...
from models.vit import CombinedLoss, build_vit_seg, count_parameters
...
# ── Run test inference after training ────────────────────────────────────────
...
def run_test_inference(best_ckpt_path: Path, data_dir: Path, test_stems, pred_dir: Path):
    if not best_ckpt_path.exists():
        ...
        return
    if not test_stems:
        print("Skipping test inference: no test samples")
        return

    from scripts.infer_vit import load_vit_model, load_image, predict_image_vit

    model = load_vit_model(str(best_ckpt_path))
    ...
    for stem in test_stems:
        ...
        _, binary_map = predict_image_vit(
            model, img,
        ...
        cv2.imwrite(str(pred_dir / f"{stem}_pred.png"), binary_map)
        saved += 1
...
def main():
    run_name  = f"vit__bs{vcfg.BATCH_SIZE}__lrdec{vcfg.LR_DECODER}__wd{vcfg.WEIGHT_DECAY}"
    run_dir   = cfg.OUTPUT_DIR / run_name
    log_dir   = run_dir / "logs"
    ckpt_dir  = run_dir / "checkpoints"
    pred_dir  = run_dir / "predictions" / "test"

    print("ViT segmentation training (SeagrassFinder best model)")
    print(f"Device  : {DEVICE}")
    print(f"Encoder : {vcfg.VIT_MODEL_NAME}")
    print(f"ImgSize : {vcfg.CROP_SIZE}")
    ...
    with open(config_log_path, "w") as f:
        json.dump({
            "model":               "vit",
            "vit_model_name":      vcfg.VIT_MODEL_NAME,
            "vit_pretrained":      vcfg.VIT_PRETRAINED,
            "vit_freeze_epochs":   vcfg.VIT_FREEZE_EPOCHS,
            "decode_channels":     vcfg.DECODE_CHANNELS,
            "in_channels":         cfg.IN_CHANNELS,
            "out_channels":        cfg.OUT_CHANNELS,
            "crop_size":           vcfg.CROP_SIZE,
            "normalization":       cfg.NORMALIZATION,
            "batch_size":          vcfg.BATCH_SIZE,
            "grad_accum_steps":    cfg.GRAD_ACCUM_STEPS,
            "epochs":              cfg.EPOCHS,
            "early_stop_patience": cfg.EARLY_STOP_PATIENCE,
            "lr_encoder":          vcfg.LR_ENCODER,
            "lr_decoder":          vcfg.LR_DECODER,
    ...
    # ── Datasets — same augmentation as UNet pipeline ────────────────────────
    ...
    train_loader = DataLoader(
        train_ds, batch_size=vcfg.BATCH_SIZE, shuffle=True,
    ...
    model = build_vit_seg(
        in_channels=cfg.IN_CHANNELS,
        out_channels=cfg.OUT_CHANNELS,
        img_size=vcfg.CROP_SIZE,
        model_name=vcfg.VIT_MODEL_NAME,
        pretrained=vcfg.VIT_PRETRAINED,
        decode_channels=vcfg.DECODE_CHANNELS,
    ).to(DEVICE)
    print(f"Parameters: {count_parameters(model):,}\n")

    # Optionally freeze ViT encoder for warmup epochs
    if vcfg.VIT_FREEZE_EPOCHS > 0:
        print(f"Freezing ViT encoder for first {vcfg.VIT_FREEZE_EPOCHS} epoch(s).")
    ...
    optimizer = optim.AdamW([
        {"params": model.encoder.parameters(),  "lr": vcfg.LR_ENCODER,
         "weight_decay": vcfg.WEIGHT_DECAY},
        {"params": model.decoder.parameters(),  "lr": vcfg.LR_DECODER,
         "weight_decay": vcfg.WEIGHT_DECAY},
    ])
    scheduler = StepLR(optimizer, step_size=cfg.LR_DECAY_STEP, gamma=cfg.LR_DECAY_RATE)
    if getattr(vcfg, "USE_COSINE_LR", False):
    ...
    for epoch in range(1, cfg.EPOCHS + 1):
        ...
        if vcfg.VIT_FREEZE_EPOCHS > 0 and epoch == vcfg.VIT_FREEZE_EPOCHS + 1:
        ...
        va_loss, va_iou = val_epoch_tiled(
            ...
            vcfg.CROP_SIZE, vcfg.TILE_STRIDE, cfg.NORMALIZATION, cfg.EVAL_THRESHOLD,
        )
        scheduler.step()
